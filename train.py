...
def predict(X, y, sigma_y, thetas, X_ms):
    y_preds = []
    L = X_ms.shape[0]
    for k in range(L):
        mu_m, A_m, K_mm_inv = phi_opt(thetas[k], X_ms[k], X, y, sigma_y)
        f_q, _ = q(X, thetas[k], X_ms[k], mu_m, A_m, K_mm_inv)
        y_preds.append(f_q)

    return y_preds

def plot_motion_types(index_to_motion, X_lists, y_lists, colors, X_ms):
...
def train(prefix, m, d, mode='code', sigma_y=0.1, use_weight=False, weights=np.array([]), colors=[]):
    ...
    if mode == 'simple':
        ...
        '''
        res = minimize(elbo_fn(X_list, y_list, indices, sigma_y, dims, mode='simple_train'),
            x0 = pack_params([jnp.array(thetas_start), jnp.array(X_ms_start)]),
            method='BFGS')
        '''
        for k in range(L):
            res = minimize(fun=elbo_fn(X_lists[k], y_lists[k], indices, sigma_y, 
        ...
        plot_motion_types(index_to_motion, X_lists, y_lists, colors, X_ms=sigmoid(np.array(X_ms)))
    ...
    if mode == 'code':
        ...
        res = minimize(fun=elbo_fn(X_list, y_list, sigma_y, dims, mode='code_train'),
...
def test(prefix, m, d, mode='code', sigma_y=0.1, params=[], max_predictions=100, use_weight=False, weights=np.array([])):
    ...
    if mode == 'simple':
        ...
        for X, y in pbar:
            ...
            res = minimize(fun=elbo_fn([X], [y], indices, sigma_y, dims, mode='simple_pred', 
