...
/* IO helper functions for reading/writing rigid objects' trajectories */
...
vector<Trajectory> read_keyframes(string input_file){
    ...
    /* Structure of input file:
        First line: num_traj num_se3 num_so3
        (Number of trajectories, number of se3 components, and number of so3 components)
        Next we have paragraphs where each corresponds to a trajectory
            First line: num_pts (number of points on this trajectory)
...
// Fcts for output a list of trajectories, where each trajectory's data include both pose and velocity (motion)
